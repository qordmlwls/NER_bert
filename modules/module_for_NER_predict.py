...
# sentences, tag, enc_tag = process_data(config.TRAINING_FILE, is_filter=False)
...
# train_sentences, val_sentences,train_tag,val_tag= train_test_split(
#     sentences,
#     tag,
#     random_state=42,
#     test_size=config.TRAIN_VAL_RATIO
# )
# data_module = NERDataModule(train_sentences, train_tag, val_sentences, val_tag, test_sentences, test_tag, config.TRAIN_BATCH_SIZE, config.VALID_BATCH_SIZE, num_tag)
# data_module.setup()
...
class NerPrdict:
    ...
    def predict_model(self):
        ...
        # label_list = enc_tag.inverse_transform(range(24)).tolist()
        model = NERModel(n_tags=self.num_tag, labels_list=label_list,
                         steps_per_epoch=len(self.train_sentences) // config.TRAIN_BATCH_SIZE, n_epochs=config.EPOCHS)
        ckpt_path = "./wandb/run-20211101_053322-1413udep/files/Named Entity Recognition/1413udep/checkpoints/epoch=9-step=99.ckpt"
        model.load_from_checkpoint(ckpt_path, n_tags=self.num_tag, labels_list=label_list,
                                   steps_per_epoch=len(self.train_sentences) // config.TRAIN_BATCH_SIZE,
        ...
        out = trainer.test(model, datamodule=self.data_module)
        return out
    def run(self):
        self.preprocess_data()
        out = self.predict_model()
        return out
